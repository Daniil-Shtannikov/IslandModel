...
public class Parameters {
    ...
    public void changeParameters(){
        ...
        if(countPredators == 0){
            countPredators = IslandSimulation.getInstance().getCountHerbivores();
        }
        if(countPlants == 0){
            countPlants = IslandSimulation.getInstance().getCountHerbivores();
        }
    ...
    private void changeIslandSize(){
        ...
        int answer = takeInt(0, 1);
        if (answer == 0) {
            System.out.println("Введите желаемый размер острова!");
            System.out.println("Количество строк (от 1 до 500):");
            int rows = takeInt(1, 500);
            System.out.println("Количество столбцов (от 1 до 500):");
